...
| `contents` | 5 | 目录 | 5–6 个短章节；4 个章节需删除原组并重建四栏 |
| `section-red` | 6 | 红底章节 | 强章节切换 |
| `statement` | 7 | 中央标语 | 品牌观点、核心结论；新增说明线前检查模板短线 |
...
| `three-columns` | 18 | 三栏文字 | 三个并列要点；正文变长时同步延长两条竖线 |
| `swot` | 19 | 红底四象限 | SWOT、四象限分析；自定义标题需调整中央竖线 |
...
| layout_id | 安全容量 | 重点检查 |
|---|---|---|
| `cover` | 1–2 行中文标题 + 1 行副标题 | 标题不要超过两行，不出现单字行 |
| `contents` | 4 栏或 2×3 共 4–6 项 | 删除空占位组，保持章节等权 |
| `statement` | 1 行引导 + 1 行主观点 + 3 项说明 | 删除重复中心短线，说明线补偿文本内边距 |
| `three-columns` | 每列标题 + 两段短正文 | 竖线覆盖最长正文并保持等高 |
| `swot` | 4 象限，每象限 1 标题 + 2 行说明 | 中央线不得进入标题区 |
| `bars-timeline` | 3 个阶段最稳定 | 阶段词放大，编号、说明和线条统一基线 |
| `closing` | Logo 模式：仅一个大 Logo；文字模式：主标题 + 简短说明，不放 Logo | 两种模式严格二选一；检查遮挡、挤压、重叠和留白 |
